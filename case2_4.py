...
if __name__ == '__main__':
    ...
    d_space_list = np.array([0.25,0.5,1])*lambda0 
    for didx,dspace in enumerate(d_space_list):
        mvdr_rmse.append([])
        music_rmse.append([])
        dbt_rmse.append([])
        '''
        ...
        for npidx in noise_powers:
            ...
            for nidx in range(Nrepeat):
                ...
                plt.legend()
                plt.savefig(f'report_figures/case2_5/snr_{npidx*-1}_d_{didx+1}_iter_{nidx}.png')
                print(f'Iteration {nidx} finished')
                #end monte carlo for
            dbt_rmse[-1].append(np.sqrt(np.sum(dbt_errors)/(Nrepeat*Nepochs)))
            mvdr_rmse[-1].append(np.sqrt(np.sum(mvdr_errors)/(Nrepeat*Nepochs)))
            music_rmse[-1].append(np.sqrt(np.sum(music_errors)/(Nrepeat*Nepochs)))
    ...
    plt.figure()
    plt.title('DBT Element Spacing')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(dbt_rmse[0]),label=f'd={d_space_list[0]}')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(dbt_rmse[1]),label=f'd={d_space_list[1]}')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(dbt_rmse[2]),label=f'd={d_space_list[2]}')
    plt.xlabel('SNR (dB)')
    plt.ylabel('RMSE (db(degrees))')
    plt.legend()
    plt.savefig('report_figures/case2a.png')
    
    plt.figure()
    plt.title('MVDR Element Spacing')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(mvdr_rmse[0]),label=f'd={d_space_list[0]}')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(mvdr_rmse[1]),label=f'd={d_space_list[1]}')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(mvdr_rmse[2]),label=f'd={d_space_list[2]}')
    plt.xlabel('SNR (dB)')
    plt.ylabel('RMSE (db(degrees))')
    plt.legend()
    plt.savefig('report_figures/case2b.png')

    plt.figure()
    plt.title('MUSIC Element Spacing')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(music_rmse[0]),label=f'd={d_space_list[0]}')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(music_rmse[1]),label=f'd={d_space_list[1]}')
    plt.plot(-1*np.asarray(noise_powers),10*np.log10(music_rmse[2]),label=f'd={d_space_list[2]}')
    plt.xlabel('SNR (dB)')
    plt.ylabel('RMSE (db(degrees))')
    plt.legend()
    plt.savefig('report_figures/case2c.png')